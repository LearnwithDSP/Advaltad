...
export const DonationImpact: React.FC = () => {
  const { donations = [] } = useLiveImpactData();
  ...
  const dynamicPrograms = IMPACT_PROGRAMS.map((prog) => {
    ...
    const directDonations = safeDonations.filter(
      (d) => d && d.program_id === prog.id && d.status === "success"
    );
    ...
    const indirectDonations = safeDonations.filter(
      (d) => d && !validProgramIds.includes(d.program_id) && d.status === "success"
    );
    ...
    const directNGN = directDonations.reduce((sum, d) => {
      ...
      return sum + ((d.amount || 0) / rate) * 1500;
    }, 0);
    ...
    const indirectNGN = indirectDonations.reduce((sum, d) => {
      const rate = CURRENCY_RATES[d.currency] || 1;
      const amountInNGN = ((d.amount || 0) / rate) * 1500;
              ...
              <div className="lg:col-span-5 flex flex-col items-center">
                <div className="w-full h-[320px] sm:h-[360px] relative flex items-center justify-center">
                  {isMounted ? (
                    ...
                    <div className="w-64 h-64 rounded-full border-4 border-slate-100 flex items-center justify-center">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="p-3 rounded-2xl bg-slate-50 text-brand-primary" style={{ color: selectedProgram.color }}>
                      <Icon name={selectedProgram.icon} size={22} />
                    </div>
                    <div>
              ...
              <div className="space-y-6">
                ...
                <div className="relative pt-4">
                  <input
                    ...
                    step="10"
                    value={simulatorAmount}
                    onChange={(e) => setSimulatorAmount(parseInt(e.target.value) || 10)}
