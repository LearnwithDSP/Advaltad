...
export function useWalletBalance(identifier?: string | null): UseWalletBalanceResult {
  ...
  const resolvedIdentifier = (
    identifier || (typeof window !== "undefined" ? localStorage.getItem(SESSION_EMAIL_KEY) : null) || ""
  ).trim();

  const fetchBalance = useCallback(async (): Promise<number> => {
    const idToUse = (
      resolvedIdentifier || (typeof window !== "undefined" ? localStorage.getItem(SESSION_EMAIL_KEY) : null) || ""
    ).trim();
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem(CACHE_KEY, String(currentBal));
      }
      setError(null);
      setLoading(false);
      return currentBal;
  ...
  useEffect(() => {
    ...
    if (!isSupabaseConfigured || !supabase || !idToUse) {
      const handleStorageLocal = (e: StorageEvent) => {
        if (active && (e.key === "advaltad_ambassadors" || e.key === "advaltad_wallets" || e.key === CACHE_KEY || e.key === SESSION_EMAIL_KEY)) {
          fetchBalance();
        }
      };
      if (typeof window !== "undefined") {
        window.addEventListener("storage", handleStorageLocal);
      }
      return () => {
        active = false;
        if (typeof window !== "undefined") {
          window.removeEventListener("storage", handleStorageLocal);
        }
      };
    }

    const channelName = `wallet-balance-${idToUse.replace(/[^a-zA-Z0-9]/g, "").slice(0, 12)}-${Date.now()}`;
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (rec && typeof rec === "object") {
            const potentialBal = rec.avu_balance !== undefined ? rec.avu_balance : rec.balance;
            if (potentialBal !== undefined && Number(potentialBal) >= 0) {
              setBalance(Number(potentialBal));
              if (typeof window !== "undefined") {
                localStorage.setItem(CACHE_KEY, String(potentialBal));
              }
            }
          }
          fetchBalance();
    ...
    const handleStorage = (e: StorageEvent) => {
      if (active && (e.key === "advaltad_ambassadors" || e.key === "advaltad_wallets" || e.key === SESSION_EMAIL_KEY)) {
        fetchBalance();
      }
    };
